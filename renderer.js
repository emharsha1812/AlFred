...
document.addEventListener("DOMContentLoaded", () => {
   ...
   * @throws {Error} If Markdown parsing or essential libraries fail.
   */
  function renderMarkdownResponse(markdownString) {
    ...
    try {
      // Check if libraries are loaded (they should be global via index.html)
      if (typeof marked === "undefined" || !marked) {
        throw new Error("Marked library is not loaded.");
      }
      // Configure marked (can customize later if needed)
      marked.setOptions({
        gfm: true, // Enable GitHub Flavored Markdown
        breaks: true, // Convert single line breaks to <br>
        // sanitize: false, // Be careful with untrusted input if you enable sanitize: true
      });
      ...
      // Highlight code blocks using client-side highlight.js
      ...
      // If parsing fails critically, return an error display element
      ...
      // You might want to throw the error again or just return the error element
      // depending on how handleSubmit should react. Let's return the element.
      return container; // Return the container with error info
      // throw error; // Alternatively, re-throw to be caught by handleSubmit
    }
  }
  ...
  async function fetchAndPopulateModels() {
    ...
    try {
      ...
      // No need to check response.ok manually if using try/catch with raise_for_status in backend
      const data = await response.json();

      if (data.error) {
        throw new Error(data.error); // Throw error reported by backend
      }
      ...
      if (fetchedModels.length > 0) {
        ...
        currentSelectedModel = modelSelect.value; // Update state
      ...
      // Re-enable controls even if there's an error/no models
      modelSelect.disabled = fetchedModels.length === 0; // Only disable if truly no models
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (data.error) {
        ...
        outputArea.appendChild(renderedElement); // Append the resulting div
        scrollToBottom();
